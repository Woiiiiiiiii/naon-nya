...
def _get_yt_service(account_id):
    ...
    # Get refresh token (env var or local file)
    ...
    if not refresh_token:
        print(f"  [SKIP] No refresh token for {account_id}")
        return None, None
    ...
    if not client_config:
        print(f"  [SKIP] No client_secret for {account_id}")
        return None, None
    ...
    try:
        ...
        print(f"  [SKIP] Auth failed for {account_id}: {e}")
        return None, None

    yt_data = build('youtube', 'v3', credentials=creds)

    # Try analytics (may not be available)
    try:
